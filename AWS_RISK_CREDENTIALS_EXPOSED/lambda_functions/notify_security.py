...
TEMPLATE = '''At {} the IAM access key {} for user {} on account {} was disabled after it was found to have been publicly exposed on the internet.
...
ERROR_MSG = '''An error occurred when attempting to disable an exposed IAM access key for one of your IAM users. Please login to your account and navigate to the Personal Health Dashboard for more details on this incident and how to resolve it.
...
def lambda_handler(event, context):
    if event.get('error-info') is not None:
        publish_msg("Security Alert: Exposed IAM Key - Error Disabling Key", ERROR_MSG)
        return
    account_id = event['account_id']
    username = event['username']
    disabled_key = event['disabled_key']
